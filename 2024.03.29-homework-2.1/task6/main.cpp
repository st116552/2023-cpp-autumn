...
public:
    ...
    bool IsDirectedGraphTournament(int vertexes);
    int getImportantVertexNumber();
...
void CGraph::initMatrixFromEdges()
{
    ...
    for (int i = 0; i < _edges; ++i)
    {
        ...
        _matrix[_edge[i].b - 1][_edge[i].a - 1] = _edge[i].w;
    }
}
...
bool CGraph::IsDirectedGraphTournament(int vertexes){
    if (_vertexes == 0) 
    {
        initMatrixFromEdges();
    }
    for (int i = 0; i < vertexes; ++i)
    {
        ...
        {
            if (((_matrix[j][i] > 0) && (_matrix[i][j] > 0)) || (!(_matrix[j][i] > 0) && !(_matrix[i][j] > 0))) 
            {
                return false;
            }
        }
...
int CGraph::getImportantVertexNumber()
{
...
int main() {
    ...
    std::cin >> n;
    CGraph gph = CGraph();
    gph.ReadEdges(n - 1, false);
    std::cout << gph.getImportantVertexNumber();
